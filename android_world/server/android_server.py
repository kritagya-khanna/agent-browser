...
async def lifespan(fast_api_app: fastapi.FastAPI):
  ...
  # Note: emulator_setup=False to avoid hanging on app installation
  # Apps should be pre-installed via install-apks service or manually
...
async def health(app_android_env: AndroidEnv):
  ...
  Verifies:
  1. Android environment is initialized
  2. Emulator has booted (sys.boot_completed = 1)
  3. CDP bridge is ready (/tmp/cdp-bridge-ready exists)
  """
  # Check 1: Environment initialized
  if not isinstance(app_android_env, interface.AsyncEnv):
    raise fastapi.HTTPException(
        status_code=503, detail="Environment not initialized"
    )

  # Check 2: Emulator booted
  try:
  ...
  if not emulator_ok:
    raise fastapi.HTTPException(
        status_code=503, detail="Emulator not booted"
    )

  # Check 3: CDP bridge ready
  cdp_ready = os.path.exists("/tmp/cdp-bridge-ready")
  if not cdp_ready:
    raise fastapi.HTTPException(
        status_code=503, detail="CDP bridge not ready"
    )
